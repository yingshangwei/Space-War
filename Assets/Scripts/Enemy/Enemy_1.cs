...
public class Enemy_1: MonoBehaviour {
	...
	private const float ShotScore = 10;   //击中当前怪物得分
	private const float DeathScore = 100; //击败当前怪物得分
	public static  float DownSpeed = 2.5f;   //下降速度
	...
	//移动
	void mMove() {
		transform.Translate (Vector3.down * DownSpeed * Time.deltaTime * ((Player.Score)/10000+1));
	...
	void OnTriggerEnter(Collider col) {
		...
		if (HP <= 0) {
			//CreatSpecialItem.Creat(1,transform.position);
			//概率生成道具
			if(Random.Range (0, 100)>=100-1) {
